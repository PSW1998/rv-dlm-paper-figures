...
#   4. Uses normal posterior bands:
#
#          theta_j,t approx N(m_j,t, C_jj,t)
#
#      so 90% bands are:
#
#          m_j,t +/- z_0.95 sqrt(C_jj,t)
#
...
def add_state_summaries(row, mt, Ct, st=None, nt=None, level=0.90):
    """
    Add posterior normal bands using C_jj directly.

    Convention:

        theta_j,t approx N(m_j,t, C_jj,t)

    so the equal-tail 90% normal band is:

        m_j,t +/- z_0.95 sqrt(C_jj,t)

    This does NOT multiply by s_t.
    """

    zcrit = norm.ppf(0.5 + level / 2.0)
    ...
    for i in range(len(mt)):
        ...
        row[f"theta{k}_lo90"] = float(mt[i] - zcrit * scales[i])
        row[f"theta{k}_hi90"] = float(mt[i] + zcrit * scales[i])
...
def fit_svdlm_fixed(data, beta=SV_BETA, delta=DELTA):
    ...
    for t_matlab, (date, r) in enumerate(data.iterrows(), start=2):
        ...
        row = add_state_summaries(row, mt, Ct, st, nt, level=0.90)
        rows.append(row)
...
def plot_sd_band_frame(df, filename, ylim=None):
    ...
    Plots:
      1. raw observed volatility proxy as a light noisy line
      2. posterior 90% SD band
      3. posterior median SD

    For FigureA1, obs_sd = sqrt(r_t^2) = |r_t|.
    For FigureA2, obs_sd = sqrt(RS_t).
    """
